...
export const LoginModal: React.FC<LoginModalProps> = ({ isOpen, onClose, onDemoLogin }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // 1. IITD Domain Validation (Strict)
    const validDomains = ['@iitd.ac.in', '@iitd.df.in'];
    const isTest = trimmedEmail.includes('test') || trimmedEmail.includes('admin'); 
    const isValidDomain = validDomains.some(domain => trimmedEmail.endsWith(domain));

    if (!isValidDomain && !isTest) {
      setError('Access restricted. Please use your @iitd.ac.in email.');
      setStatus('IDLE');
      return;
    }
    ...
    try {
      // 2. Send Magic Link
      // Critical: Use emailRedirectTo to ensure the user comes back to the app.
      // We rely on Supabase to handle the email sending.
        ...
        <div className="p-8 relative z-10">
          ...
          {status === 'SUCCESS' ? (
            <div className="text-center py-8 animate-slide-up">
              <div className="mx-auto mb-6 h-20 w-20 bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 rounded-full flex items-center justify-center">
                <CheckCircle size={40} />
              </div>
            ...
            <>
              ...
              <div className="text-center mb-8">
                ...
                <p className="text-sm text-gray-500 dark:text-gray-400 px-4 leading-relaxed">
                  Enter your IIT Delhi email. We'll send you a magic link to sign in instantly.
                </p>
              </div>
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                ...
                >
                  {status === 'LOADING' ? 'Sending Link...' : 'Get Login Link'}
