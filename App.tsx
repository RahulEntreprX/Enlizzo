...
import { fetchListings, updateListingStatus, fetchSavedItems, updateUserProfile, createListing, toggleSavedItem } from './services/db';
...
const GeometricBackground = memo(({ variant, isDark }: { variant: 'landing' | 'default', isDark: boolean }) => {
  ...
  const beamColorMid = isDark ? 'rgba(34, 211, 238, 0.4)' : 'rgba(99, 102, 241, 0.7)'; // Reduced opacity
...
const App = () => {
  ...
  useEffect(() => {
    const handleUrlSync = () => {
       ...
       if (productParam) {
           ...
           if (['marketplace', 'listing', 'profile', 'admin'].includes(pageParam)) {
               setCurrentView(pageParam as ViewState);
           } else {
               // Fallback will be handled by the Auth Effect
           }
  ...
  useEffect(() => {
    ...
    if (user && currentView === 'landing') {
        ...
        try {
          ...
          console.error("Failed to clean URL on logout", e);
        }
    }
  }, [user, currentView]);
  ...
  const loadData = useCallback(async (forceLoadingState = false) => {
    try {
      ...
      // Pass isDemo flag to ensure strict separation of data
  ...
  }, [currentView, loadData, products.length]);
  ...
  const handleNavigate = (view: ViewState, scrollToTop: boolean = false) => {
    ...
    try {
      ...
      if (view === 'marketplace' || view === 'landing') {
          url.searchParams.delete('page');
      } else {
  ...
  const handleLogout = async () => {
      // Immediate UI update to avoid race conditions
      setCurrentView('landing');
      ...
      return (
          <div className={`min-h-screen flex items-center justify-center transition-colors duration-300 ${isDark ? 'bg-[#030712]' : 'bg-[#F0F4F8]'}`}>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center z-20">
                  ...
                  <div className="text-indigo-500 font-mono text-sm animate-pulse">INITIALIZING CAMPUS LINK...</div>
              </div>
          </div>
      );
  }
                ...
                <div className="flex items-center justify-center min-h-[50vh]">
                   <div className="flex flex-col items-center gap-4">
                      ...
                      <p className="text-gray-500 text-sm font-mono animate-pulse">
                        {products.length === 0 ? 'Loading Item...' : 'Item not found'}
                      </p>
